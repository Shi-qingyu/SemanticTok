"""
Reference: https://github.com/hustvl/LightningDiT/blob/main/transport/transport.py
"""
...
class Transport:
    ...
    def sample_in_range(self, mu, sigma, target_size, range_min=0, range_max=0.5):
        ...
        while len(samples) < target_size:
            ...
            filtered_samples = generated_samples[
                (generated_samples >= range_min) & (generated_samples <= range_max)
            ]
    ...
    def sample(self, x1, sp_timesteps=None, shifted_mu=0):
        ...
        if not self.use_lognorm:
            if self.partitial_train is not None and th.rand(1) < self.partial_ratio:
                t = (
                    th.rand((x1.shape[0],)) * (self.partitial_train[1] - self.partitial_train[0])
            ...
            if not self.shift_lg:
                if self.partitial_train is not None and th.rand(1) < self.partial_ratio:
                    t = self.sample_in_range(
                        0,
                        1,
                        x1.shape[0],
                        range_min=self.partitial_train[0],
                ...
                assert (
                    self.partitial_train is None
                ), "Shifted lognormal distribution is not compatible with partial training"
    ...
    def training_losses(
        ...
        raw_model_output = model(xt, t, **model_kwargs)
        if isinstance(raw_model_output, tuple):
            model_output = raw_model_output[0]
        else:
            model_output = raw_model_output
        B, *_, C = xt.shape
        # the channel dim is the one with least size
        channel_dim = min([i for i in range(1, len(xt.shape))], key=lambda x: xt.size(x))
        ...
        if self.model_type == ModelType.VELOCITY:
            ...
            if self.use_cosine_loss:
                terms["cos_loss"] = mean_flat(
                    1 - th.nn.functional.cosine_similarity(model_output, ut, dim=channel_dim)
                )
        else:
